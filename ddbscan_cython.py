...
def ddbscan(X, eps=0.5, epsransac=1.0, min_samples=5, metric='minkowski', metric_params=None,
    ...
    if metric == 'precomputed' and sparse.issparse(X):
        ...
        masked_indptr = masked_indptr2[X.indptr[1:-1]]
    ...
    #labels = np.zeros([X.shape[0],2], dtype = np.intp)
    labels = np.full(X.shape[0], -1, dtype=np.intp)
    
    # A list of all core samples found.
    core_samples = np.asarray(n_neighbors >= min_samples, dtype=np.uint8)
    labels = ddbscan_inner_cython.ddbscaninner(X, core_samples, neighborhoods, neighborhoods2, labels)
